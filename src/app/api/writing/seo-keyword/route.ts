...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = (await request.json()) as SeoKeywordBody;

    if (!body.analysisText?.trim()) {
      return NextResponse.json({ error: "analysisText is required" }, { status: 400 });
    }
    ...
    const result = await generateText({
      ...
      prompt: PROMPTS.seoKeywordExtraction(body.analysisText),
    });

    return NextResponse.json({ keyword: result.text.trim() });
  } catch (error) {
